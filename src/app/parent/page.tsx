...
type Tab = "today" | "homework" | "growth" | "comments" | "schedule" | "attendance";
    ...
    const fetchStudentData = useCallback(async (query: string, pin?: string) => {
        ...
        try {
            if (pin) {
                ...
                // Homework — may fail due to RLS
                try {
                    const [rHw, rSub] = await Promise.all([
                        supabase.from("homework").select("*").order("due_date", { ascending: false }).limit(20),
                        supabase.from("submissions").select("*").eq("student_id", sid).order("submitted_at", { ascending: false }).limit(50),
                    ]);
    ...
    useEffect(() => {
        ...
        const ch = supabase.channel("parent-monitor-v2")
            ...
            .on("postgres_changes", { event: "UPDATE", schema: "public", table: "profiles", filter: `id=eq.${student.id}` }, (p) => {
                ...
                setStudent(prev => prev ? { ...prev, level: d.level || prev.level, total_xp: d.total_xp || d.xp || prev.total_xp } : prev);
            })
            .subscribe();
                            ...
                            {homeworkError ? (
                                ...
                                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                                    {homeworkList.map(hw => {
                                        ...
                                        const isOverdue = !sub && new Date(hw.due_date) < new Date();
                                        ...
                                        return (
                                            <div key={hw.id} style={{ ...gc, padding: 20, borderLeft: `4px solid ${isSubmitted ? "#22c55e" : isOverdue ? "#ef4444" : BLUE.primary}` }}>
                                                ...
                                                <div style={{ display: "flex", gap: 16, fontSize: 12, color: "#94a3b8" }}>
                                                    ...
                                                    {hw.max_score > 0 && (
                                                        <span>
                                                            <span className="material-symbols-outlined" style={{ fontSize: 14, verticalAlign: "middle", marginRight: 4 }}>grade</span>
                                                            배점: {hw.max_score}점
                                                        </span>
                                                    )}
                                                </div>
                                                {isSubmitted && isGraded && sub.score !== null && (
                                                    <div style={{ marginTop: 10, padding: "8px 14px", borderRadius: 10, background: "#f0fdf4", display: "flex", alignItems: "center", gap: 8 }}>
                                                        ...
                                                        <span style={{ fontSize: 13, fontWeight: 700, color: "#16a34a" }}>
                                                            {sub.score}점 {hw.max_score > 0 ? `/ ${hw.max_score}점` : ""}
                                                        </span>
                                                        {sub.feedback && (
